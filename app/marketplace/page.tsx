...
const EMOJI_MAP: Record<string, string> = {
  'EPI': '🦺', 'Formation': '🎓', 'Detection': '📡',
  'Service HSE': '🛡️', 'Premiers secours': '🩺',
  'Signalisation': '⚠️', 'Incendie': '🔥', 'Autre': '🔧'
}
...
export default function MarketplacePage() {
  ...
  const filtered = annonces.filter(a => {
    const matchSearch = !search || a.titre?.toLowerCase().includes(search.toLowerCase()) || a.description?.toLowerCase().includes(search.toLowerCase())
              ...
              {filtered.map(a => (
                <div key={a.id} className="card group overflow-hidden flex flex-col">
                  <div className="bg-navy-700 h-36 flex items-center justify-center relative overflow-hidden">
                    ...
                    <button
                      onClick={() => setLiked(prev => prev.includes(a.id) ? prev.filter(i => i !== a.id) : [...prev, a.id])}
